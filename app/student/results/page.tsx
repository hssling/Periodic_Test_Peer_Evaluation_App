...
export default async function StudentResultsPage() {
  ...
  // Get all attempts with test info and evaluations
    ...
    evaluatedAttempts.length > 0
      ? Math.round(
          evaluatedAttempts.reduce(
            (acc, a) =>
              acc + ((a.final_score || 0) / (a.test?.total_marks || 1)) * 100,
            0,
      ...
      ? Math.round(
          Math.max(
            ...evaluatedAttempts.map(
              (a) => ((a.final_score || 0) / (a.test?.total_marks || 1)) * 100,
            ),
          ),
        )
      : 0;
            ...
            <div className="space-y-3">
              {allAttempts.map((attempt) => {
                const percentage = attempt.test?.total_marks
                  ? Math.round(
                      ((attempt.final_score || 0) / attempt.test.total_marks) *
                        100,
                    )
                  : 0;
