-- Migration: Allocation guards, evaluation validation, and indexes

-- Indexes for faster dashboards
CREATE INDEX IF NOT EXISTS idx_allocations_attempt_status
  ON allocations(attempt_id, status);

CREATE INDEX IF NOT EXISTS idx_allocations_evaluator_status
  ON allocations(evaluator_id, status);

-- Snapshot scoring context for evaluation items
ALTER TABLE evaluation_items
  ADD COLUMN IF NOT EXISTS max_score INTEGER,
  ADD COLUMN IF NOT EXISTS score_percent NUMERIC;

-- Allocation guard + logging
...
BEGIN
    ...
    IF v_available < v_evaluators_needed THEN
        ...
        RAISE EXCEPTION 'Not enough evaluators: needed %, available %', v_evaluators_needed, v_available;
    END IF;
...
-- Evaluation validation + snapshot scores
...
BEGIN
    ...
    IF EXISTS (
        ...
        AND ei.score > q.max_marks
    ) THEN
        RAISE EXCEPTION 'One or more scores exceed the maximum for the question.';
    END IF;
    ...
    -- Snapshot max score + percent per item
    UPDATE evaluation_items ei
    SET 
        max_score = q.max_marks,
        score_percent = CASE
            WHEN q.max_marks > 0 THEN (ei.score::numeric / q.max_marks) * 100
            ELSE 0
        END
    FROM questions q
    WHERE ei.evaluation_id = p_evaluation_id
    AND q.id = ei.question_id;
