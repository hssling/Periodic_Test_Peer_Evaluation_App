-- Migration: Enforce batch-year-only matching for tests, attempts, and allocations
...
UPDATE profiles
SET batch = normalize_batch_year(batch)
WHERE batch IS DISTINCT FROM normalize_batch_year(batch);

UPDATE tests
SET target_batch = normalize_batch_year(target_batch)
WHERE target_batch IS DISTINCT FROM normalize_batch_year(target_batch);
...
CREATE POLICY "tests_select_published" ON tests
    ...
    USING (
        ...
        OR (
            ...
            AND (
                target_batch IS NULL
                OR normalize_batch_year(target_batch) IS NOT DISTINCT FROM normalize_batch_year(
                    (SELECT batch FROM profiles WHERE user_id = auth.uid())
                )
            )
        )
    );
...
CREATE POLICY "attempts_insert_own" ON attempts
    ...
    WITH CHECK (
        ...
        EXISTS (
            ...
            JOIN profiles p ON p.id = attempts.student_id
            WHERE t.id = test_id
            AND t.status = 'active'
            AND NOW() BETWEEN t.start_at AND t.end_at
            AND (
                t.target_batch IS NULL
                OR normalize_batch_year(t.target_batch) IS NOT DISTINCT FROM normalize_batch_year(p.batch)
            )
        )
    );
...
BEGIN
    SELECT 
        a.test_id, a.student_id, normalize_batch_year(p.batch),
        normalize_batch_year(t.target_batch), t.evaluators_per_submission, t.same_batch_only, 
    ...
    candidates AS (
        ...
        AND (v_target_batch IS NULL OR normalize_batch_year(p.batch) IS NOT DISTINCT FROM v_target_batch)
        AND (NOT v_same_batch_only OR normalize_batch_year(p.batch) IS NOT DISTINCT FROM v_student_batch)
        AND NOT EXISTS (
